  ...
  if (categories.length === 0) {
    container.innerHTML = `
      <div style="grid-column: 1/-1; text-align: center; padding: 60px 20px;">
        ...
        <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--text-main, #1e293b); margin-bottom: 8px;">No Categories Yet</h3>
        <p style="color: var(--text-muted, #64748b); font-size: 0.92rem; max-width: 440px; margin: 0 auto 20px auto;">
          Add supermarket departments and categories (Fruits, Vegetables, Dairy, Bakery, etc.) to organize products.
        </p>
        <button class="btn-primary" onclick="openAddCategoryModal()" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; font-weight: 700; cursor: pointer;">
          + Add New Category
        </button>
      </div>
    `;
  ...
  if (filtered.length === 0) {
    container.innerHTML = `
      <div style="grid-column: 1/-1; text-align: center; padding: 48px 20px;">
        ...
        <h4 style="font-weight: 700; color: var(--text-main, #334155); margin-bottom: 6px;">No categories found</h4>
        <p style="color: var(--text-muted, #94a3b8); font-size: 0.88rem;">No category matches "${searchVal}". Try a different keyword.</p>
      </div>
    `;
    return;
  }
