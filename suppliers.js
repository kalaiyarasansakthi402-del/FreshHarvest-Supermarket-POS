    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="7" style="text-align: center; padding: 60px 20px;">
          ...
          <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--text-main, #1e293b); margin-bottom: 8px;">No Suppliers Registered Yet</h3>
          <p style="color: var(--text-muted, #64748b); font-size: 0.92rem; max-width: 440px; margin: 0 auto 20px auto;">
            Add your vendor and wholesale supplier contacts to manage procurement and purchase orders.
          </p>
          <button class="btn-primary" onclick="openAddSupplierModal()" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; font-weight: 700; cursor: pointer;">
            + Add New Supplier
          </button>
        </td>
      </tr>
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="7" style="text-align: center; padding: 48px 20px;">
          ...
          <h4 style="font-weight: 700; color: var(--text-main, #334155); margin-bottom: 6px;">No suppliers found</h4>
          <p style="color: var(--text-muted, #94a3b8); font-size: 0.88rem;">No supplier matches "${searchVal}". Try a different name, person, phone, or email.</p>
        </td>
      </tr>
    `;
    return;
...
function openEditSupplierModal(id) {
  const suppliers = getSuppliers();
  const sup = suppliers.find(s => String(s.id) === String(id) || s.id == id);
  if (!sup) return;
...
function handleSupplierSubmit(e) {
  ...
  if (editId) {
    const idx = suppliers.findIndex(s => String(s.id) === String(editId) || s.id == editId);
    if (idx !== -1) {
      suppliers[idx] = {
        ...suppliers[idx],
...
function deleteSupplier(id) {
  ...
  if (confirm(`Are you sure you want to delete supplier "${sup.name}"?`)) {
    const updated = suppliers.filter(s => String(s.id) !== String(id) && s.id != id);
