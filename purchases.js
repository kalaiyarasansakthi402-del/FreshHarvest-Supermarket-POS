...
function getPurchases() {
  if (window.DataStore && typeof DataStore.get === 'function') {
    const list = DataStore.get('PURCHASES');
    if (Array.isArray(list)) return list;
  }
  return JSON.parse(localStorage.getItem("freshHarvestPurchases") || localStorage.getItem("purchasesList") || "[]");
}

function savePurchases(list) {
  if (window.DataStore && typeof DataStore.set === 'function') {
    DataStore.set('PURCHASES', list);
  } else {
    localStorage.setItem("freshHarvestPurchases", JSON.stringify(list));
    localStorage.setItem("purchasesList", JSON.stringify(list));
  }
}

function getSuppliers() {
  if (window.DataStore && typeof DataStore.getSuppliers === 'function') {
    return DataStore.getSuppliers();
  }
  return JSON.parse(localStorage.getItem("freshHarvestSuppliers") || localStorage.getItem("suppliersList") || "[]");
}
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="8" style="text-align: center; padding: 60px 20px;">
          ...
          <h3 style="font-size: 1.25rem; font-weight: 800; color: var(--text-main, #1e293b); margin-bottom: 8px;">No Purchase Orders Yet</h3>
          <p style="color: var(--text-muted, #64748b); font-size: 0.92rem; max-width: 440px; margin: 0 auto 20px auto;">
            Create purchase orders to order fresh inventory from suppliers and auto-restock products upon delivery.
          </p>
          <button class="btn-primary" onclick="openNewPOModal()" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; font-weight: 700; cursor: pointer;">
            + New Purchase Order
          </button>
        </td>
      </tr>
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="8" style="text-align: center; padding: 48px 20px;">
          ...
          <h4 style="font-weight: 700; color: var(--text-main, #334155); margin-bottom: 6px;">No purchase orders found</h4>
          <p style="color: var(--text-muted, #94a3b8); font-size: 0.88rem;">No purchase order matches "${searchVal}". Try a different PO number, supplier, or product name.</p>
        </td>
      </tr>
    `;
    return;
